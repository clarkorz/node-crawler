...
exports.Crawler = function(options) {
    ...
    self.options = _.extend({
        timeout:        60000,
        jQuery:         true,
        jQueryUrl:      path.resolve(__dirname,"../vendor/jquery-2.0.3.min.js"),
        maxConnections: 10,
        priorityRange:  10,
        priority:       5,
        retries:        3,
    ...
    self.request = function(opts) {
        ...
        if (!ropts.headers["User-Agent"] && ropts.userAgent) {
        ...
        var req = request(_.pick.apply(this,[ropts].concat(requestArgs)), function(error,response,body) {
            ...
            var decode = function (error, body) {
                if (error) return self.onContent(error, opts);
            ...
            switch ((response.headers['content-encoding'] || '').trim().toLowerCase()) {
                case 'gzip':
                    zlib.gunzip(body, decode);
                    break;
                case 'deflate':
                    zlib.deflate(body, decode);
                    break;
                default:
                    self.onContent(error,opts,response,false);
                    break;
    ...
    self.queue = function(item) {
        ...
        if (toQueue.headers && self.options.headers) {
            _.defaults(toQueue.headers, self.options.headers);
        }
